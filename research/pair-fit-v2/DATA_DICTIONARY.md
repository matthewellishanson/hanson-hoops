...
The Git-ignored `curated/phase3e-r2/` directory contains exactly nine generated artifacts: three CSV files and six JSON files.
...
- `estimator_feature_manifest.json` records the exact ordered 45-feature estimator allowlist, frozen Ridge design metadata, prohibited-column checks, symmetric-feature contract, slot-swap proof, and training-only preprocessing policy.
...
- `input_fingerprints.json` records every referenced cache file hash and the before/after immutability result.
- `artifact_hashes.json` is the bookkeeping manifest of deterministic serialized-byte hashes for the substantive payload artifacts.
- `summary.json` records the deterministic construction summary, prerequisite anchors, artifact references, and Phase 3E-R2 construction classification.

No value learned from a 2024-25 predictor or target is stored as preprocessing state.
...
- `POSS`: cumulative team possessions during the pair's shared minutes, directly returned by the Advanced response. It describes team exposure while both players are on court, not either player's possessions. It is the documented and empirically validated denominator for standard `OFF_RATING`; the response does not expose a separate opponent-possession field. It is not a target or player-quality feature. It is additive across the validated disjoint Charlotte windows, but low values make rate targets unstable; missing, nonnumeric, or nonpositive values remain audited and are target-ineligible.
- `OFF_RATING`: standard rate describing team points scored per 100 team possessions while both players share the court, including the other teammates, opponents, and context. It is directly returned, not derived by this project, and is a component/audit check for the selected `NET_RATING` target, not a predictor. Phase 1F empirically validates recomposition across the two Charlotte windows using `POSS`; direct full-season validity is supported.
- `DEF_RATING`: standard rate describing opponent points scored per 100 opponent possessions while both players share the court. It is directly returned, not an individual or on/off statistic and not derived by this project. It is a component/audit check for the selected `NET_RATING` target, not a predictor. Direct full-season validity is supported, but it is not safely recomposable with returned team `POSS` because the required opponent-possession denominator is absent.
- `NET_RATING`: standard rate equal to directly returned `OFF_RATING - DEF_RATING`, subject to one-decimal publication rounding. It describes the team's efficiency margin while both players share the court and is not an individual rating or on/off differential. It is the selected Phase 3B prediction target, preserved unmodified, with primary eligibility `POSS >= 150` and a separately retained `POSS >= 100` sensitivity population. All unfiltered and excluded raw observations remain preserved for audit. `POSS`, pair minutes, and target-period ratings remain excluded from predictors. The older pre-Phase-3 description of this target and eligibility as provisional is historical only.
- `E_OFF_RATING`: directly returned NBA estimated offensive-rate variant for the team while the pair shares the court. It is rate-based and estimated, not a project prediction. The exact public formula or denominator is unresolved in available authoritative evidence. It is not currently approved as the selected target and is not empirically recomposable with returned `POSS`; it remains a future candidate.
- `E_DEF_RATING`: directly returned NBA estimated defensive-rate variant for the team while the pair shares the court. It is rate-based and estimated, not project-derived. Its exact formula or denominator is unresolved, it is not currently approved as the selected target, and it is not empirically recomposable with returned `POSS`; it remains a future candidate.
- `E_NET_RATING`: directly returned NBA estimated net-rate variant for the team while the pair shares the court. It is internally coherent with `E_OFF_RATING - E_DEF_RATING` within one-decimal rounding, but the exact estimation method is unresolved. It is not currently approved as the selected target and is not empirically recomposable with returned `POSS`; it remains a future candidate.
...
- `PLUS_MINUS`: cumulative, standard Base statistic equal to team points minus opponent points while the pair shares the court. It is directly returned, not project-derived, and describes team rather than individual performance. It has no possession denominator, is not a rate, net rating, or on/off statistic, and is not an approved target. It is additive across the validated disjoint Charlotte windows and permits opponent points to be derived as `PTS - PLUS_MINUS`, but cannot supply the missing opponent-possession denominator needed for defensive-rating reconstruction.

Phase 1F requires three independent status checks for every candidate: direct full-season semantic/internal validity, safe cross-window recomposability from available fields, and reliability at the row's exposure. Failure of one does not automatically determine the other two.
...
### Phase 3B curated fields

`phase3b_poss_ge_150.csv` is the primary curated selection and `phase3b_poss_ge_100.csv` is its possession-floor sensitivity selection. Both are reproducible Git-ignored CSVs. `player_1_*` and `player_2_*` are source/audit slots only, never direct estimator columns. `player_[1|2]_history_profile_season`, `player_[1|2]_history_gap`, and `player_[1|2]_history_missing` identify closest strict-prior profile selection within a three-season maximum. `missing_player_count`, `history_status`, and `strict_complete_history_eligible` retain missing-history evidence.

`target_net_rating` is the unaltered target. `pair_possessions`, `pair_base_minutes`, exposure flags, candidate weight columns, target component audit fields, identifiers, provenance, endpoint-boundary flags, and pandemic flags are metadata rather than predictors. Candidate weight columns are equal row, square-root possessions, and possessions capped at 300; uncapped linear possession weight is excluded.

`player_[1|2]_shot_*` preserves residual-v1 zone FGM/FGA, overall and unclassified counts, source-null flags, attempted-zone flags, and classified coverage. A source-null paired count normalizes structurally to 0/0 while retaining the null indicator; zero-attempt efficiency is undefined. Left and right corner counts may form one combined-corner feature; source `Corner 3` is validation-only. The manifest defines all later symmetric estimator transforms and prohibited predictors; see `PHASE3B_CURATION_FEATURE_SPEC.md`.

Player `GP`, `TOTAL_MIN`, and overall FGA are preserved as reliability/audit inputs, not primary estimator predictors. A player traded-history indicator is null for invalid `TEAM_COUNT`, rather than coerced to zero. The only retained pair trade feature is a known-only 0--2 count. The seven-component shot-distribution L1 predictor uses overall-FGA shares for restricted area, non-restricted paint, mid-range, combined corners, above-the-break, backcourt, and unclassified residual; it is null when either required profile/vector is unavailable.
...
Phase 2B subsequently completed all 60 2023-24 pair assets, Phase 2C completed all 60 2022-23 pair assets plus two 2021-22 player assets, Phase 2D completed all 60 2021-22 pair assets plus two 2020-21 player assets, and Phase 2E completed the same 62-asset pattern for each target season from 2020-21 through 2014-15. No new raw field or semantic definition was accepted: all releases match the approved pair contracts, and every later player-source season matches reviewed 69-column `phase2a.player-base.v2`. Totals `MIN` remains a season-total-minute diagnostic while Per100Possessions `MIN` is rate-normalized; neither has an approved positive eligibility threshold or automatic role as a model feature. Phase 2E retains valid zero-possession rows and does not select their later treatment.
...
- Phase 1E demonstrated Charlotte window-population recovery and additive-total reconstruction. Phase 1F later supported direct standard full-season target semantics while leaving window reconstruction and reliability policy distinct and unresolved. Separately authorized immutable raw-only historical acquisition may defer threshold selection; curated materialization and modeling remain prohibited until target, exposure, missing-history, and validation policies are resolved.
