...
- `OFF_RATING`: standard rate describing team points scored per 100 team possessions while both players share the court, including the other teammates, opponents, and context. It is directly returned, not derived by this project, and is a target component rather than the currently provisional primary target. Phase 1F empirically validates recomposition across the two Charlotte windows using `POSS`; direct full-season validity is supported. Reliability at low exposure remains unresolved.
- `DEF_RATING`: standard rate describing opponent points scored per 100 opponent possessions while both players share the court. It is directly returned, not an individual or on/off statistic and not derived by this project. It is a target component, not the provisional primary target. Direct full-season validity is supported, but it is not safely recomposable with returned team `POSS` because the required opponent-possession denominator is absent. Low-exposure reliability remains unresolved.
- `NET_RATING`: standard rate equal to directly returned `OFF_RATING - DEF_RATING`, subject to one-decimal publication rounding. It describes the team's efficiency margin while both players share the court and is not an individual rating or on/off differential. It is directly returned rather than project-derived and remains the provisional standard target candidate, not a final selected target. All 5,297 full-season values are internally coherent within `0.1`; direct validity is supported. It is not safely recomposable from the available windows because its components use different denominators. Low-exposure reliability and the final eligibility policy remain unresolved.
