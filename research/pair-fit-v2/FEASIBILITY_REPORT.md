...
Current status: Phase 1A is complete. Its four-team, cache-replayable pilot confirmed identical Base/Advanced schemas and one-to-one joins for 736 observations while finding materially different prior-history coverage across the bounded sample; that pattern does not establish roster type as the cause or a league-wide relationship. Phase 1B defined the design-only raw JSON → curated Parquet → DuckDB architecture and hardened validation contracts. Phase 1C subsequently implemented the persisted raw manifest and completed the 30-team/60-asset request set: all 60 assets verify, all 5,297 Base and Advanced returned pair keys match one-to-one, and both complete-season gates pass. Phase 1D proved Charlotte's full-season response non-exhaustive. Phase 1E recovered a 257-key Charlotte union containing all 250 full-season keys plus seven recovered-only pairs and reproduced additive totals exactly. Phase 1F supports direct full-season standard target semantics while leaving reliability policy unresolved. Phase 2A demonstrates identical 2023-24 pair schemas and one-to-one reconciliation for 880 five-team canary observations. Phase 2A.1 subsequently reviewed the exact two-column historical player addition, promoted request 11 without refetching, verified the one separately authorized Totals asset, supported Totals `MIN` semantics, and completed 2022-23 prior joins. Phase 2B then stopped before HTTP on its first new identity because of a local transport-allowlist defect. The consumed attempt was not retried and 49 new identities remain unattempted. Classification: `2023-24 raw acquisition incomplete; historical expansion blocked`. No materialization, threshold selection, complete historical expansion, or modeling has occurred.
...
5. **Phase 1E bounded recovery feasibility: demonstrated for Charlotte population/additive totals; targets unresolved**
   - The early/late union contains 257 Charlotte keys: all 250 full-season keys plus seven recovered-only keys, including the three Phase 1D proving pairs.
   - All supported additive Base fields and Advanced `POSS` reproduce the immutable full-season values for 250/250 comparable pairs.
   - Possession-weighted `OFF_RATING` is within `0.1` for every pair, but 9 `DEF_RATING` and 10 `NET_RATING` rows exceed `0.2`; the exceptions were not proven to be only documented rounding artifacts.
   - Known omissions remain eligible at the evaluated `POSS >= 1` and `POSS >= 5` sensitivities and cease at `POSS >= 10`. This does not select a threshold or rule out unknown higher-exposure omissions.
   - The Charlotte gate therefore stopped the sequence before Philadelphia. The bounded result is `window recovery demonstrated; target recomposition unresolved`, not exhaustiveness.

6. **Phase 1F target semantics: direct standard target supported; reliability policy unresolved**
   - All 5,297 directly returned full-season `NET_RATING` values reconcile with directly returned offense minus defense within `0.1`.
   - Returned team `POSS` supports offensive reconstruction, while no separate opponent-possession denominator is exposed; unavailable denominator/precision prevents safe defensive/net window reconstruction but does not invalidate the direct target.
   - All estimated fields are complete and internally coherent, but their exact methodology is unresolved and they are not selected.
   - Charlotte temporal stability and league-wide extremes improve materially with exposure, but the high-exposure Charlotte sample is too narrow to establish policy.
   - Classification: `direct full-season target semantics supported; reliability threshold unresolved`.

7. **Still required before modeling or historical expansion**
   - Finalize the target variant and exposure policy using broader temporal evidence; no final target or threshold is selected.
   - Do not ordinarily average partial-window Advanced ratings; any future recovery requires validated numerator/denominator behavior against known full-season rows.
...
**Phase 1A: COMPLETE. Phase 1B architecture and ingestion-contract design: complete for its approved design scope. Phase 1C: complete 30-team raw request set. Phase 1D: Charlotte population non-exhaustiveness proven. Phase 1E: Charlotte window recovery/additive totals demonstrated. Phase 1F: direct standard target semantics supported; reliability threshold unresolved.** The earlier statement that the remaining-team manifest had not been executed is superseded by Phase 1C, and Phase 1E's unresolved reconstruction result no longer implies that directly returned full-season `NET_RATING` is invalid.
...
Raw-only historical acquisition may be considered under separate authorization while threshold selection is deferred, provided the endpoint-returned population limitation is explicit and no curated/model-ready artifact is created. Modeling remains no-go until the listed policies and validation evidence are satisfied.
...
Subsequent Phase 1A work established bounded four-team schema, join and coverage behavior, Phase 1B established the architecture contract, and Phase 1C established request-set completeness and returned-row integrity for all 30 teams. Phase 1D proved Charlotte's full-season response omitted valid season pairs. Phase 1E demonstrated a larger observed Charlotte window-union population and exact supported additive totals. Phase 1F established that direct standard full-season target semantics remain supported despite unsafe defensive/net window reconstruction, while reliability policy remains unresolved. The project still has **not** established an exhaustive recoverable 30-team population, multi-season consistency, or predictive model feasibility.
