"""CLI for the bounded Phase 2C 2022-23 raw-season acquisition."""
...
from pair_fit_v2.phase2c_raw_season import (
    analyze_release,
    create_store,
    dry_run_plan,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if sum((args.initialize, args.preview, args.live_acquisition, args.analyze)) != 1:
        raise SystemExit("Choose exactly one Phase 2C operation")
    ...
    if args.initialize:
        ...
        output = run_acquisition(store, live_acquisition=True, delay_seconds=args.delay_seconds)
    else:
